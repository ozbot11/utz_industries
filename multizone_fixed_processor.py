"""
Multi-Zone Targeted Fix Processor
EXACT original code + different blending strategies for different areas:
- Face: Larger detection area + better crop positioning
- Arms: Expanded edge smoothing for limb blending  
- Chest: Keep current working method
"""
...
class MultiZoneFixProcessor:
    """Multi-zone processor with different blending strategies for face vs arms vs chest"""
    
    def __init__(self):
        print("🎭 Multi-Zone Targeted Fix Processor")
        print("=" * 55)
        print("🎯 Different blending strategies for face, arms, chest")
        ...
        print("✅ Multi-zone processor ready!")
    ...
    def _detect_and_crop_face_improved(self, image: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[Tuple]]:
        """IMPROVED face detection and cropping - larger area, better positioning"""
        try:
            # Convert to grayscale for face detection
            gray = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
            
            # Use OpenCV face detection with multiple cascades for better detection
            ...
            faces2 = face_cascade2.detectMultiScale(gray, 1.05, 3)
            
            # Combine results and get best face
            all_faces = []
            if len(faces1) > 0:
            ...
            if len(all_faces) > 0:
                ...
                # LARGER padding for better context and blending (35% instead of 20%)
                padding = int(min(w, h) * 0.35)
                x = max(0, x - padding)
                y = max(0, y - padding)
                ...
                if x + w <= image.shape[1] and y + h <= image.shape[0]:
                    ...
                    print(f"   🎯 IMPROVED face detection: {w}x{h} with 35% padding")
    ...
    def _paste_face_back_multizone(self, original_image: np.ndarray, new_face: np.ndarray, 
                                  face_coords: Tuple[int, int, int, int]) -> np.ndarray:
        """MULTI-ZONE blending - different strategies for different areas"""
        try:
            x, y, w, h = face_coords
            ...
            # Create sophisticated multi-zone mask
            mask = np.zeros((h, w), dtype=np.float32)
            
            # ZONE 1: Core facial area (strong replacement)
            # Create elliptical core for main facial features
            center_x, center_y = w // 2, h // 2
            core_w = int(w * 0.6)  # 60% width
            core_h = int(h * 0.65) # 65% height
            
            # Core face region gets full replacement
            cv2.ellipse(mask, (center_x, center_y), (core_w // 2, core_h // 2), 0, 0, 360, 1.0, -1)
            
            # ZONE 2: Transition area (gradient blending)
            # Create larger ellipse for transition
            transition_w = int(w * 0.85)  # 85% width
            transition_h = int(h * 0.85)  # 85% height
            
            # Create transition mask
            transition_mask = np.zeros((h, w), dtype=np.float32)
            cv2.ellipse(transition_mask, (center_x, center_y), (transition_w // 2, transition_h // 2), 0, 0, 360, 1.0, -1)
            
            # Subtract core from transition to get gradient area
            gradient_area = transition_mask - mask
            gradient_area = np.clip(gradient_area, 0, 1)
            
            # ZONE 3: Edge smoothing (for arm/hair boundaries)
            # Larger edge smoothing for arm areas
            edge_size = 8  # Larger edge smoothing for arms
            
            # Apply distance transform for smooth gradient in transition area
            dist_transform = cv2.distanceTransform((gradient_area * 255).astype(np.uint8), cv2.DIST_L2, 5)
            if dist_transform.max() > 0:
                dist_transform = dist_transform / dist_transform.max()
                gradient_area = gradient_area * dist_transform
            
            # Combine core + gradient for final mask
            final_mask = mask + gradient_area * 0.7  # Reduce gradient strength
            final_mask = np.clip(final_mask, 0, 1)
            
            # Apply additional edge smoothing for arm boundaries
            edge_mask = np.ones((h, w), dtype=np.float32)
            edge_mask[:edge_size, :] *= 0.3    # Top edge (hair boundary)
            edge_mask[-edge_size:, :] *= 0.5   # Bottom edge
            edge_mask[:, :edge_size] *= 0.2    # Left edge (arm/hair boundary)  
            edge_mask[:, -edge_size:] *= 0.2   # Right edge (arm/hair boundary)
            
            # Apply gaussian blur to edge transitions
            edge_mask = cv2.GaussianBlur(edge_mask, (15, 15), 0)
            
            # Combine masks
            final_mask = final_mask * edge_mask
            
            # Apply heavy smoothing for seamless transitions
            ...
            # Apply blending
            blended_region = face_float * mask_3d + original_region * (1 - mask_3d)
            
            # Color tone matching for seamless integration
            face_mean = np.mean(face_float, axis=(0, 1))
            orig_mean = np.mean(original_region, axis=(0, 1))
            
            # Apply subtle color correction in transition areas
            color_diff = orig_mean - face_mean
            transition_areas = (1 - mask_3d) * mask_3d  # Areas where blending occurs
            color_correction = color_diff * transition_areas * 0.2
            ...
            print(f"   ✨ MULTI-ZONE blending: Core face + gradient + edge smoothing")
            return result
            
        except Exception as e:
            print(f"   Multi-zone blending error: {e}")
    ...
    def _generate_five_stage_human(self, skeleton: np.ndarray, skeleton_scope: str, 
        ...
        """Generate AI human - ALL STAGES IDENTICAL TO ORIGINAL + multi-zone blending"""
        if not all([self.stage1_pipe, self.stage2_pipe, self.stage3_pipe, self.stage4_pipe, self.stage5_pipe]):
            return None
        
        try:
            print(f"🎨 MULTI-ZONE Five-stage generation for: {image_name}")
            ...
            # STAGE 5: Face generation - IMPROVED DETECTION + MULTI-ZONE BLENDING
            print("   👤 Stage 5: Face generation + MULTI-ZONE blending...")
            
            # IMPROVED face detection and cropping
            face_crop, face_coords = self._detect_and_crop_face_improved(stage4_image)
            ...
            if face_crop is not None and face_coords is not None:
                ...
                with torch.autocast(self.device):
                    stage5_result = self.stage5_pipe(
                        ...
                        image=face_pil,
                        strength=0.8,  # SAME AS ORIGINAL WORKING VERSION
                        num_inference_steps=45,  # SAME AS ORIGINAL WORKING VERSION
                        guidance_scale=15.0,  # SAME AS ORIGINAL WORKING VERSION
                ...
                # MULTI-ZONE blending for face/arms/hair boundaries
                final_image = self._paste_face_back_multizone(stage4_image, new_face, face_coords)
                print(f"   ✅ Stage 5 + MULTI-ZONE blending complete in {stage5_time:.1f}s")
            ...
            print(f"🌟 MULTI-ZONE Five-stage complete in {total_time:.1f}s total")
            print(f"✨ Multi-zone blending: face + arms + edge smoothing!")
    ...
    def process_folder(self, input_folder: str, output_folder: str, character_file: str):
        ...
        try:
            ...
            print(f"\n🌟 MULTI-ZONE TARGETED PROCESSING")
            print("=" * 60)
            print(f"📁 Input: {input_folder}")
            print(f"📁 Output: {output_folder}")
            print(f"👤 Character: {character.name}")
            print(f"📸 Images: {len(image_files)}")
            print(f"🎯 Method: Multi-zone blending (face/arms/chest)")
            print(f"✨ Goal: Fix face AND arm boundaries")
                ...
                try:
                    ...
                    if skeleton_result is not None:
                        ...
                        if ai_result is not None:
                            # Save results
                            output_file = output_path / f"multizone_human_{image_file.stem}.png"
                            cv2.imwrite(str(output_file), cv2.cvtColor(ai_result, cv2.COLOR_RGB2BGR))
                            
                            skeleton_file = output_path / f"multizone_skeleton_{image_file.stem}.png"
            ...
            print(f"\n🌟 MULTI-ZONE PROCESSING COMPLETE!")
            print("=" * 55)
            print(f"⏰ Total time: {total_time/60:.1f} minutes")
            print(f"✅ Successful: {self.successful_generations}")
            print(f"❌ Failed: {self.failed_generations}")
            print(f"📊 Success rate: {success_rate:.1f}%")
            print(f"🎭 Method: Multi-zone blending for face + arms + chest")
            print(f"✨ Targeted fixes for all boundary issues!")
...
def main():
    ...
    print("Multi-Zone Targeted Fix Processor")
    print("=" * 55)
    print("🎯 Different blending strategies for face vs arms vs chest")
    ...
    processor = MultiZoneFixProcessor()
    ...
    output_folder = input("📁 Output folder: ").strip() or "multizone_humans"
    ...
    print(f"\n✨ Running MULTI-ZONE version - different strategies for face/arms/chest...")
